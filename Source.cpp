...
string StrMultInt(const string& num, int umn) {
    ...
    { 
        int digit = num[i] - '0'; // Преобразование символа в число
        int a = digit * umn + ost; // Переменная для поразрядного умножения
        result.push_back((a % 10) + '0'); // Цифра текущего результата
        ost = a / 10; // Остаток
    }
    while (ost > 0) 
    {
...
int proverka() {
    ...
    while (true) {
        ...
        if (good) {
            int value = stoi(input); // stoi для преобразования str в int
            if (value >= 10) {
                return value;
            }
        }
